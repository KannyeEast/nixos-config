  ...
  flake.modules.nixos.networking =
    ...
    let
        ...
        lib.flatten (
          lib.mapAttrsToList (
            ...
            let
              path = "${prefix}/${name}";
            in
            if lib.isAttrs value then flattenPaths path value else [ path ]
          ) attrs
        );
      ...
      pathToEnv =
        string:
        lib.toUpper (
          lib.stringAsChars (c: if builtins.match "[A-Za-z0-9]" c != null then c else "_") string
        );
        ...
        (mkIf (wifiSecrets != { }) {
          ...
          networking.networkmanager.ensureProfiles = {
            ...
            profiles = lib.mapAttrs (
              ...
              // lib.mapAttrs (
                section: keys: lib.mapAttrs (key: _: "$" + pathToEnv "wifi/${name}/${section}/${key}") keys
              ) sections
            ) wifiSecrets;
          };
        })
