...
in
{
  flake.modules.homeManager.browserTabs =
    ...
    let
      ...
      # Generated by `flavours apply` and committed. A host that has never had
      # a scheme applied will not have one, so fall back to a neutral grey
      # rather than failing the whole eval on a missing file.
