...
let
  inherit (lib)
    concatMapAttrs concatStringsSep filterAttrs
    hasPrefix hasSuffix mapAttrs mapAttrs' mapAttrsToList mkOption
    nameValuePair optionalAttrs removeSuffix types;
in
{
  flake.modules.nixos.backup =
    ...
    let
      ...
      rmSuffix = mapAttrs' (name: value: 
        nameValuePair (removeSuffix "_unencrypted" name) value);
      
      envOf = data: data.env or { };
      secretEnv = filterAttrs (k: _:
        !hasSuffix "_unencrypted" k);
      
      mkEnv = name: env:
        concatStringsSep "\n" (mapAttrsToList (k: value:
          if hasSuffix "_unencrypted" k
          then "${removeSuffix "_unencrypted" k}='${value}'"
          else "${k}='${config.sops.placeholder."backup/${name}/env/${k}"}'"
        ) env);
    in
    {
      options = {
      ...
      config = {
        sops.secrets = concatMapAttrs (name: data:
          { "backup/${name}/password" = { }; }
          // optionalAttrs (data ? healthcheck) { "backup/${name}/healthcheck" = { }; }
          // concatMapAttrs (k: _:
            { "backup/${name}/env/${k}" = { }; }
          ) (secretEnv (envOf data))
        ) targets;
        
        sops.templates = mapAttrs' (name: data:
          nameValuePair "restic-${name}.env" { content = mkEnv name (envOf data); }
        ...
        services.restic.backups = mapAttrs (name: data:
        let
        ...
        systemd.services = concatMapAttrs (name: data:
